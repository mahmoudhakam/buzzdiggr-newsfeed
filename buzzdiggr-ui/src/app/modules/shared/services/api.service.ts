...
export class ApiService {
  ...
  getProfileData(userId) {
    return new Observable((observer) => {
      ...
      const user = (this.sharedData.buzzdiggrnce.getValue() || {})[userId];
      if ( !user) {
        const message = `user ${this.sharedData.userId.getValue()} not found on this buzzdiggrnce, please login again`;
        console.log(message)
        alert(message);
      ...
      const working = [];
      const breaking = [];
      if( user.buzzdiggrnce) {
        user.buzzdiggrnce.forEach(row => {
